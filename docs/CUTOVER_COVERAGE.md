...
| Reports | `/api/reports/summary`, `/reports/:key`, `/reports/:key/csv` | `/api/v1/reports/...` (same) | **READY** | routed; platform computes summary metrics + CSV/PDF for orders/products/expenses/purchases/debts/staff/full. PDFs render a branded cover page, per-section summary, bar charts, paginated tables, and footers |
